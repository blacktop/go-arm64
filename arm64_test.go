...
func Test_decompose_basic(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "{{lsl|lsr}}	w3, w2, #0",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x43, 0x7c, 0x00, 0x53}),
				address:          0,
			},
			want: "{{lsl|lsr}}	w3, w2, #0",
			wantErr: false,
		},
		{
			name: "lsl	w9, w10, #31",
		...
		{
			name: "fmov	s2, #{{0.12500000|1.250*e-01}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x02, 0x10, 0x28, 0x1e}),
				address:          0,
			},
			want: "fmov	s2, #{{0.12500000|1.250*e-01}}",
			wantErr: false,
		},
		{
			name: "ldr	w0, #1048572",
		...
		{
			name: "{{ands	xzr,|tst}} x18, #0xcccccccccccccccc",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x5f, 0xe6, 0x02, 0xf2}),
				address:          0,
			},
			want: "{{ands	xzr,|tst}} x18, #0xcccccccccccccccc",
			wantErr: false,
		},
		{
		...
		{
			name: "{{ands	xzr,|tst}} x3, #0xaaaaaaaaaaaaaaaa",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x7f, 0xf0, 0x01, 0xf2}),
				address:          0,
			},
			want: "{{ands	xzr,|tst}} x3, #0xaaaaaaaaaaaaaaaa",
			wantErr: false,
		},
		{
			name: "{{ands	xzr,|tst}} xzr, #0x5555555555555555",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xff, 0xf3, 0x00, 0xf2}),
				address:          0,
			},
			want: "{{ands	xzr,|tst}} xzr, #0x5555555555555555",
			wantErr: false,
		},
		{
			name: "mov	w3, #983055",
		...
		{
			name: "msr	{{teecr32_el1|TEECR32_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x00, 0x12, 0xd5}),
				address:          0,
			},
			want: "msr	{{teecr32_el1|TEECR32_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{osdtrrx_el1|OSDTRRX_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x00, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{osdtrrx_el1|OSDTRRX_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{mdccint_el1|MDCCINT_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x02, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{mdccint_el1|MDCCINT_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{mdscr_el1|MDSCR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x02, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{mdscr_el1|MDSCR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{osdtrtx_el1|OSDTRTX_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x03, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{osdtrtx_el1|OSDTRTX_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgdtr_el0|DBGDTR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x04, 0x13, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgdtr_el0|DBGDTR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgdtrtx_el0|DBGDTRTX_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x05, 0x13, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgdtrtx_el0|DBGDTRTX_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{oseccr_el1|OSECCR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x06, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{oseccr_el1|OSECCR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgvcr32_el2|DBGVCR32_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x07, 0x14, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgvcr32_el2|DBGVCR32_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr0_el1|DBGBVR0_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x00, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr0_el1|DBGBVR0_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr1_el1|DBGBVR1_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x01, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr1_el1|DBGBVR1_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr2_el1|DBGBVR2_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x02, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr2_el1|DBGBVR2_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr3_el1|DBGBVR3_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x03, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr3_el1|DBGBVR3_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr4_el1|DBGBVR4_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x04, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr4_el1|DBGBVR4_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr5_el1|DBGBVR5_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x05, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr5_el1|DBGBVR5_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr6_el1|DBGBVR6_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x06, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr6_el1|DBGBVR6_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr7_el1|DBGBVR7_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x07, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr7_el1|DBGBVR7_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr8_el1|DBGBVR8_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x08, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr8_el1|DBGBVR8_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr9_el1|DBGBVR9_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x09, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr9_el1|DBGBVR9_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr10_el1|DBGBVR10_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x0a, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr10_el1|DBGBVR10_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr11_el1|DBGBVR11_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x0b, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr11_el1|DBGBVR11_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgbvr12_el1|DBGBVR12_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x0c, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgbvr12_el1|DBGBVR12_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbg
EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x04, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr4_el1|DBGWCR4_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr5_el1|DBGWCR5_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x05, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr5_el1|DBGWCR5_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr6_el1|DBGWCR6_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x06, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr6_el1|DBGWCR6_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr7_el1|DBGWCR7_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x07, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr7_el1|DBGWCR7_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr8_el1|DBGWCR8_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x08, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr8_el1|DBGWCR8_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr9_el1|DBGWCR9_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x09, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr9_el1|DBGWCR9_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr10_el1|DBGWCR10_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x0a, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr10_el1|DBGWCR10_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr11_el1|DBGWCR11_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x0b, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr11_el1|DBGWCR11_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr12_el1|DBGWCR12_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x0c, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr12_el1|DBGWCR12_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr13_el1|DBGWCR13_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x0d, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr13_el1|DBGWCR13_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr14_el1|DBGWCR14_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x0e, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr14_el1|DBGWCR14_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgwcr15_el1|DBGWCR15_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x0f, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgwcr15_el1|DBGWCR15_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{teehbr32_el1|TEEHBR32_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x10, 0x12, 0xd5}),
				address:          0,
			},
			want: "msr	{{teehbr32_el1|TEEHBR32_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{oslar_el1|OSLAR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x10, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{oslar_el1|OSLAR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{osdlr_el1|OSDLR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x13, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{osdlr_el1|OSDLR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgprcr_el1|DBGPRCR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x14, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgprcr_el1|DBGPRCR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgclaimset_el1|DBGCLAIMSET_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0x78, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgclaimset_el1|DBGCLAIMSET_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dbgclaimclr_el1|DBGCLAIMCLR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0x79, 0x10, 0xd5}),
				address:          0,
			},
			want: "msr	{{dbgclaimclr_el1|DBGCLAIMCLR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{csselr_el1|CSSELR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x00, 0x1a, 0xd5}),
				address:          0,
			},
			want: "msr	{{csselr_el1|CSSELR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{vpidr_el2|VPIDR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x00, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{vpidr_el2|VPIDR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{vmpidr_el2|VMPIDR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0x00, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{vmpidr_el2|VMPIDR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{sctlr_el1|SCTLR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x10, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{sctlr_el1|SCTLR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{sctlr_el2|SCTLR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x10, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{sctlr_el2|SCTLR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{sctlr_el3|SCTLR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x10, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{sctlr_el3|SCTLR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{actlr_el1|ACTLR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x10, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{actlr_el1|ACTLR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{actlr_el2|ACTLR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x10, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{actlr_el2|ACTLR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{actlr_el3|ACTLR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x10, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{actlr_el3|ACTLR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cpacr_el1|CPACR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x10, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{cpacr_el1|CPACR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{hcr_el2|HCR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x11, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{hcr_el2|HCR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{scr_el3|SCR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x11, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{scr_el3|SCR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{mdcr_el2|MDCR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x11, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{mdcr_el2|MDCR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{sder32_el3|SDER32_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x11, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{sder32_el3|SDER32_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cptr_el2|CPTR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x11, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{cptr_el2|CPTR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cptr_el3|CPTR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x11, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{cptr_el3|CPTR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{hstr_el2|HSTR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0x11, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{hstr_el2|HSTR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{hacr_el2|HACR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0x11, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{hacr_el2|HACR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{mdcr_el3|MDCR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x13, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{mdcr_el3|MDCR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{ttbr0_el1|TTBR0_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x20, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{ttbr0_el1|TTBR0_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{ttbr0_el2|TTBR0_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x20, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{ttbr0_el2|TTBR0_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{ttbr0_el3|TTBR0_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x20, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{ttbr0_el3|TTBR0_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{ttbr1_el1|TTBR1_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x20, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{ttbr1_el1|TTBR1_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{tcr_el1|TCR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x20, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{tcr_el1|TCR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{tcr_el2|TCR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x20, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{tcr_el2|TCR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{tcr_el3|TCR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x20, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{tcr_el3|TCR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{vttbr_el2|VTTBR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x21, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{vttbr_el2|VTTBR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{vtcr_el2|VTCR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x21, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{vtcr_el2|VTCR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dacr32_el2|DACR32_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x30, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{dacr32_el2|DACR32_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{spsr_el1|SPSR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x40, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{spsr_el1|SPSR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{spsr_el2|SPSR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x40, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{spsr_el2|SPSR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{spsr_el3|SPSR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x40, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{spsr_el3|SPSR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{elr_el1|ELR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x40, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{elr_el1|ELR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{elr_el2|ELR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x40, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{elr_el2|ELR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{elr_el3|ELR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x40, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{elr_el3|ELR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{sp_el0|SP_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x41, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{sp_el0|SP_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{sp_el1|SP_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x41, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{sp_el1|SP_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{sp_el2|SP_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x41, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{sp_el2|SP_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{SPSel|SPSEL}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x42, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{SPSel|SPSEL}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{nzcv|NZCV}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x42, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{nzcv|NZCV}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{daif|DAIF}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x42, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{daif|DAIF}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{SPSR_irq|SPSR_IRQ}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x43, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{SPSR_irq|SPSR_IRQ}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{SPSR_abt|SPSR_ABT}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x43, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{SPSR_abt|SPSR_ABT}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{SPSR_und|SPSR_UND}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x43, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{SPSR_und|SPSR_UND}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{SPSR_fiq|SPSR_FIQ}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0x43, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{SPSR_fiq|SPSR_FIQ}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{fpcr|FPCR}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x44, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{fpcr|FPCR}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{fpsr|FPSR}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x44, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{fpsr|FPSR}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dspsr_el0|DSPSR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x45, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{dspsr_el0|DSPSR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{dlr_el0|DLR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x45, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{dlr_el0|DLR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{ifsr32_el2|IFSR32_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x50, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{ifsr32_el2|IFSR32_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{afsr0_el1|AFSR0_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x51, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{afsr0_el1|AFSR0_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{afsr0_el2|AFSR0_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x51, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{afsr0_el2|AFSR0_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{afsr0_el3|AFSR0_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x51, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{afsr0_el3|AFSR0_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{afsr1_el1|AFSR1_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x51, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{afsr1_el1|AFSR1_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{afsr1_el2|AFSR1_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x51, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{afsr1_el2|AFSR1_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{afsr1_el3|AFSR1_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x51, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{afsr1_el3|AFSR1_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{esr_el1|ESR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x52, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{esr_el1|ESR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{esr_el2|ESR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x52, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{esr_el2|ESR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{esr_el3|ESR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x52, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{esr_el3|ESR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{fpexc32_el2|FPEXC32_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x53, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{fpexc32_el2|FPEXC32_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{far_el1|FAR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x60, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{far_el1|FAR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{far_el2|FAR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x60, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{far_el2|FAR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{far_el3|FAR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x60, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{far_el3|FAR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{hpfar_el2|HPFAR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0x60, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{hpfar_el2|HPFAR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{par_el1|PAR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x74, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{par_el1|PAR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmcr_el0|PMCR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x9c, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmcr_el0|PMCR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmcntenset_el0|PMCNTENSET_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x9c, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmcntenset_el0|PMCNTENSET_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmcntenclr_el0|PMCNTENCLR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x9c, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmcntenclr_el0|PMCNTENCLR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmovsclr_el0|PMOVSCLR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0x9c, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmovsclr_el0|PMOVSCLR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmselr_el0|PMSELR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0x9c, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmselr_el0|PMSELR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmccntr_el0|PMCCNTR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x9d, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmccntr_el0|PMCCNTR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmxevtyper_el0|PMXEVTYPER_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x9d, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmxevtyper_el0|PMXEVTYPER_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmxevcntr_el0|PMXEVCNTR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x9d, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmxevcntr_el0|PMXEVCNTR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmuserenr_el0|PMUSERENR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0x9e, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmuserenr_el0|PMUSERENR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmintenset_el1|PMINTENSET_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0x9e, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmintenset_el1|PMINTENSET_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmintenclr_el1|PMINTENCLR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0x9e, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmintenclr_el1|PMINTENCLR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmovsset_el0|PMOVSSET_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0x9e, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmovsset_el0|PMOVSSET_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{mair_el1|MAIR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xa2, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{mair_el1|MAIR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{mair_el2|MAIR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xa2, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{mair_el2|MAIR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{mair_el3|MAIR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xa2, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{mair_el3|MAIR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{amair_el1|AMAIR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xa3, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{amair_el1|AMAIR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{amair_el2|AMAIR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xa3, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{amair_el2|AMAIR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{amair_el3|AMAIR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xa3, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{amair_el3|AMAIR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{vbar_el1|VBAR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xc0, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{vbar_el1|VBAR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{vbar_el2|VBAR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xc0, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{vbar_el2|VBAR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{vbar_el3|VBAR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xc0, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{vbar_el3|VBAR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{rmr_el1|RMR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xc0, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{rmr_el1|RMR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{rmr_el2|RMR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xc0, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{rmr_el2|RMR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{rmr_el3|RMR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xc0, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{rmr_el3|RMR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{contextidr_el1|CONTEXTIDR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xd0, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{contextidr_el1|CONTEXTIDR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{tpidr_el0|TPIDR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xd0, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{tpidr_el0|TPIDR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{tpidr_el2|TPIDR_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xd0, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{tpidr_el2|TPIDR_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{tpidr_el3|TPIDR_EL3}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xd0, 0x1e, 0xd5}),
				address:          0,
			},
			want: "msr	{{tpidr_el3|TPIDR_EL3}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{tpidrro_el0|TPIDRRO_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xd0, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{tpidrro_el0|TPIDRRO_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{tpidr_el1|TPIDR_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xd0, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{tpidr_el1|TPIDR_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntfrq_el0|CNTFRQ_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe0, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntfrq_el0|CNTFRQ_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntvoff_el2|CNTVOFF_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xe0, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntvoff_el2|CNTVOFF_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntkctl_el1|CNTKCTL_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe1, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntkctl_el1|CNTKCTL_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cnthctl_el2|CNTHCTL_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe1, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{cnthctl_el2|CNTHCTL_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntp_tval_el0|CNTP_TVAL_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe2, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntp_tval_el0|CNTP_TVAL_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cnthp_tval_el2|CNTHP_TVAL_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe2, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{cnthp_tval_el2|CNTHP_TVAL_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntps_tval_el1|CNTPS_TVAL_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe2, 0x1f, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntps_tval_el1|CNTPS_TVAL_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntp_ctl_el0|CNTP_CTL_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xe2, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntp_ctl_el0|CNTP_CTL_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cnthp_ctl_el2|CNTHP_CTL_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xe2, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{cnthp_ctl_el2|CNTHP_CTL_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntps_ctl_el1|CNTPS_CTL_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xe2, 0x1f, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntps_ctl_el1|CNTPS_CTL_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntp_cval_el0|CNTP_CVAL_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xe2, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntp_cval_el0|CNTP_CVAL_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cnthp_cval_el2|CNTHP_CVAL_EL2}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xe2, 0x1c, 0xd5}),
				address:          0,
			},
			want: "msr	{{cnthp_cval_el2|CNTHP_CVAL_EL2}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntps_cval_el1|CNTPS_CVAL_EL1}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xe2, 0x1f, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntps_cval_el1|CNTPS_CVAL_EL1}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntv_tval_el0|CNTV_TVAL_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe3, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntv_tval_el0|CNTV_TVAL_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntv_ctl_el0|CNTV_CTL_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xe3, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntv_ctl_el0|CNTV_CTL_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{cntv_cval_el0|CNTV_CVAL_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xe3, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{cntv_cval_el0|CNTV_CVAL_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr0_el0|PMEVCNTR0_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe8, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr0_el0|PMEVCNTR0_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr1_el0|PMEVCNTR1_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xe8, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr1_el0|PMEVCNTR1_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr2_el0|PMEVCNTR2_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xe8, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr2_el0|PMEVCNTR2_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr3_el0|PMEVCNTR3_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xe8, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr3_el0|PMEVCNTR3_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr4_el0|PMEVCNTR4_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xe8, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr4_el0|PMEVCNTR4_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr5_el0|PMEVCNTR5_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xe8, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr5_el0|PMEVCNTR5_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr6_el0|PMEVCNTR6_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0xe8, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr6_el0|PMEVCNTR6_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr7_el0|PMEVCNTR7_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0xe8, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr7_el0|PMEVCNTR7_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr8_el0|PMEVCNTR8_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xe9, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr8_el0|PMEVCNTR8_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr9_el0|PMEVCNTR9_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xe9, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr9_el0|PMEVCNTR9_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr10_el0|PMEVCNTR10_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xe9, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr10_el0|PMEVCNTR10_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr11_el0|PMEVCNTR11_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xe9, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr11_el0|PMEVCNTR11_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr12_el0|PMEVCNTR12_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xe9, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr12_el0|PMEVCNTR12_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr13_el0|PMEVCNTR13_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xe9, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr13_el0|PMEVCNTR13_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr14_el0|PMEVCNTR14_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0xe9, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr14_el0|PMEVCNTR14_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr15_el0|PMEVCNTR15_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0xe9, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr15_el0|PMEVCNTR15_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr16_el0|PMEVCNTR16_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xea, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr16_el0|PMEVCNTR16_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr17_el0|PMEVCNTR17_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xea, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr17_el0|PMEVCNTR17_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr18_el0|PMEVCNTR18_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xea, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr18_el0|PMEVCNTR18_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr19_el0|PMEVCNTR19_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xea, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr19_el0|PMEVCNTR19_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr20_el0|PMEVCNTR20_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xea, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr20_el0|PMEVCNTR20_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr21_el0|PMEVCNTR21_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xea, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr21_el0|PMEVCNTR21_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr22_el0|PMEVCNTR22_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0xea, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr22_el0|PMEVCNTR22_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr23_el0|PMEVCNTR23_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0xea, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr23_el0|PMEVCNTR23_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr24_el0|PMEVCNTR24_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xeb, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr24_el0|PMEVCNTR24_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr25_el0|PMEVCNTR25_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xeb, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr25_el0|PMEVCNTR25_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr26_el0|PMEVCNTR26_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xeb, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr26_el0|PMEVCNTR26_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr27_el0|PMEVCNTR27_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xeb, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr27_el0|PMEVCNTR27_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr28_el0|PMEVCNTR28_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xeb, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr28_el0|PMEVCNTR28_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr29_el0|PMEVCNTR29_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xeb, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr29_el0|PMEVCNTR29_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevcntr30_el0|PMEVCNTR30_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0xeb, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevcntr30_el0|PMEVCNTR30_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmccfiltr_el0|PMCCFILTR_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0xef, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmccfiltr_el0|PMCCFILTR_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper0_el0|PMEVTYPER0_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xec, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper0_el0|PMEVTYPER0_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper1_el0|PMEVTYPER1_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xec, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper1_el0|PMEVTYPER1_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper2_el0|PMEVTYPER2_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xec, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper2_el0|PMEVTYPER2_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper3_el0|PMEVTYPER3_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xec, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper3_el0|PMEVTYPER3_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper4_el0|PMEVTYPER4_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xec, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper4_el0|PMEVTYPER4_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper5_el0|PMEVTYPER5_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xec, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper5_el0|PMEVTYPER5_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper6_el0|PMEVTYPER6_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0xec, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper6_el0|PMEVTYPER6_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper7_el0|PMEVTYPER7_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0xec, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper7_el0|PMEVTYPER7_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper8_el0|PMEVTYPER8_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xed, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper8_el0|PMEVTYPER8_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper9_el0|PMEVTYPER9_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xed, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper9_el0|PMEVTYPER9_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper10_el0|PMEVTYPER10_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xed, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper10_el0|PMEVTYPER10_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper11_el0|PMEVTYPER11_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xed, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper11_el0|PMEVTYPER11_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper12_el0|PMEVTYPER12_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xed, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper12_el0|PMEVTYPER12_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper13_el0|PMEVTYPER13_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xed, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper13_el0|PMEVTYPER13_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper14_el0|PMEVTYPER14_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0xed, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper14_el0|PMEVTYPER14_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper15_el0|PMEVTYPER15_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0xed, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper15_el0|PMEVTYPER15_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper16_el0|PMEVTYPER16_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xee, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper16_el0|PMEVTYPER16_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper17_el0|PMEVTYPER17_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xee, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper17_el0|PMEVTYPER17_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper18_el0|PMEVTYPER18_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xee, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper18_el0|PMEVTYPER18_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper19_el0|PMEVTYPER19_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xee, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper19_el0|PMEVTYPER19_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper20_el0|PMEVTYPER20_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xee, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper20_el0|PMEVTYPER20_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper21_el0|PMEVTYPER21_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xee, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper21_el0|PMEVTYPER21_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper22_el0|PMEVTYPER22_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0xee, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper22_el0|PMEVTYPER22_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper23_el0|PMEVTYPER23_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xec, 0xee, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper23_el0|PMEVTYPER23_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper24_el0|PMEVTYPER24_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xef, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper24_el0|PMEVTYPER24_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper25_el0|PMEVTYPER25_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2c, 0xef, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper25_el0|PMEVTYPER25_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper26_el0|PMEVTYPER26_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x4c, 0xef, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper26_el0|PMEVTYPER26_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper27_el0|PMEVTYPER27_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x6c, 0xef, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper27_el0|PMEVTYPER27_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper28_el0|PMEVTYPER28_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x8c, 0xef, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper28_el0|PMEVTYPER28_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper29_el0|PMEVTYPER29_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xef, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper29_el0|PMEVTYPER29_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{pmevtyper30_el0|PMEVTYPER30_EL0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xcc, 0xef, 0x1b, 0xd5}),
				address:          0,
			},
			want: "msr	{{pmevtyper30_el0|PMEVTYPER30_EL0}}, x12",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{teecr32_el1|TEECR32_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x00, 0x32, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{teecr32_el1|TEECR32_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{osdtrrx_el1|OSDTRRX_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x00, 0x30, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{osdtrrx_el1|OSDTRRX_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mdccsr_el0|MDCCSR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x01, 0x33, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mdccsr_el0|MDCCSR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mdccint_el1|MDCCINT_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x02, 0x30, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mdccint_el1|MDCCINT_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {
 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{ccsidr_el1|CCSIDR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{csselr_el1|CSSELR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x00, 0x3a, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{csselr_el1|CSSELR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{vpidr_el2|VPIDR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x00, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{vpidr_el2|VPIDR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{clidr_el1|CLIDR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x00, 0x39, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{clidr_el1|CLIDR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{ctr_el0|CTR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x00, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{ctr_el0|CTR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mpidr_el1|MPIDR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0x00, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mpidr_el1|MPIDR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{vmpidr_el2|VMPIDR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0x00, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{vmpidr_el2|VMPIDR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{revidr_el1|REVIDR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0x00, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{revidr_el1|REVIDR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{aidr_el1|AIDR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0x00, 0x39, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{aidr_el1|AIDR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{dczid_el0|DCZID_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0x00, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{dczid_el0|DCZID_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_pfr0_el1|ID_PFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x01, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_pfr0_el1|ID_PFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_pfr1_el1|ID_PFR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x01, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_pfr1_el1|ID_PFR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_dfr0_el1|ID_DFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x01, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_dfr0_el1|ID_DFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_afr0_el1|ID_AFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0x01, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_afr0_el1|ID_AFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_mmfr0_el1|ID_MMFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0x01, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_mmfr0_el1|ID_MMFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_mmfr1_el1|ID_MMFR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0x01, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_mmfr1_el1|ID_MMFR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_mmfr2_el1|ID_MMFR2_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0x01, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_mmfr2_el1|ID_MMFR2_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_mmfr3_el1|ID_MMFR3_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0x01, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_mmfr3_el1|ID_MMFR3_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_mmfr4_el1|ID_MMFR4_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0x02, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_mmfr4_el1|ID_MMFR4_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_mmfr5_el1|ID_MMFR5_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0x03, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_mmfr5_el1|ID_MMFR5_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_isar0_el1|ID_ISAR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x02, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_isar0_el1|ID_ISAR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_isar1_el1|ID_ISAR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x02, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_isar1_el1|ID_ISAR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_isar2_el1|ID_ISAR2_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x02, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_isar2_el1|ID_ISAR2_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_isar3_el1|ID_ISAR3_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0x02, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_isar3_el1|ID_ISAR3_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_isar4_el1|ID_ISAR4_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0x02, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_isar4_el1|ID_ISAR4_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_isar5_el1|ID_ISAR5_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0x02, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_isar5_el1|ID_ISAR5_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mvfr0_el1|MVFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x03, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mvfr0_el1|MVFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mvfr1_el1|MVFR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x03, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mvfr1_el1|MVFR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mvfr2_el1|MVFR2_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x03, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mvfr2_el1|MVFR2_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64pfr0_el1|ID_AA64PFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x04, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64pfr0_el1|ID_AA64PFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64pfr1_el1|ID_AA64PFR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x04, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64pfr1_el1|ID_AA64PFR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64dfr0_el1|ID_AA64DFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x05, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64dfr0_el1|ID_AA64DFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64dfr1_el1|ID_AA64DFR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x05, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64dfr1_el1|ID_AA64DFR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64afr0_el1|ID_AA64AFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0x05, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64afr0_el1|ID_AA64AFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64afr1_el1|ID_AA64AFR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0x05, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64afr1_el1|ID_AA64AFR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64isar0_el1|ID_AA64ISAR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x06, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64isar0_el1|ID_AA64ISAR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64isar1_el1|ID_AA64ISAR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x06, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64isar1_el1|ID_AA64ISAR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64mmfr0_el1|ID_AA64MMFR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x07, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64mmfr0_el1|ID_AA64MMFR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{id_aa64mmfr1_el1|ID_AA64MMFR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x07, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{id_aa64mmfr1_el1|ID_AA64MMFR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{sctlr_el1|SCTLR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x10, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{sctlr_el1|SCTLR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{sctlr_el2|SCTLR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x10, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{sctlr_el2|SCTLR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{sctlr_el3|SCTLR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x10, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{sctlr_el3|SCTLR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{actlr_el1|ACTLR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x10, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{actlr_el1|ACTLR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{actlr_el2|ACTLR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x10, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{actlr_el2|ACTLR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{actlr_el3|ACTLR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x10, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{actlr_el3|ACTLR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cpacr_el1|CPACR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x10, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cpacr_el1|CPACR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{hcr_el2|HCR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x11, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{hcr_el2|HCR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{scr_el3|SCR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x11, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{scr_el3|SCR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mdcr_el2|MDCR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x11, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mdcr_el2|MDCR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{sder32_el3|SDER32_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x11, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{sder32_el3|SDER32_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cptr_el2|CPTR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x11, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cptr_el2|CPTR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cptr_el3|CPTR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x11, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cptr_el3|CPTR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{hstr_el2|HSTR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0x11, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{hstr_el2|HSTR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{hacr_el2|HACR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0x11, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{hacr_el2|HACR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mdcr_el3|MDCR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x13, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mdcr_el3|MDCR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{ttbr0_el1|TTBR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x20, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{ttbr0_el1|TTBR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{ttbr0_el2|TTBR0_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x20, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{ttbr0_el2|TTBR0_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{ttbr0_el3|TTBR0_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x20, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{ttbr0_el3|TTBR0_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{ttbr1_el1|TTBR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x20, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{ttbr1_el1|TTBR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{tcr_el1|TCR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x20, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{tcr_el1|TCR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{tcr_el2|TCR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x20, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{tcr_el2|TCR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{tcr_el3|TCR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x20, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{tcr_el3|TCR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{vttbr_el2|VTTBR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x21, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{vttbr_el2|VTTBR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{vtcr_el2|VTCR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x21, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{vtcr_el2|VTCR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{dacr32_el2|DACR32_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x30, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{dacr32_el2|DACR32_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{spsr_el1|SPSR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x40, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{spsr_el1|SPSR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{spsr_el2|SPSR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x40, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{spsr_el2|SPSR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{spsr_el3|SPSR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x40, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{spsr_el3|SPSR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{elr_el1|ELR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x40, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{elr_el1|ELR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{elr_el2|ELR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x40, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{elr_el2|ELR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{elr_el3|ELR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x40, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{elr_el3|ELR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{sp_el0|SP_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x41, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{sp_el0|SP_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{sp_el1|SP_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x41, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{sp_el1|SP_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{sp_el2|SP_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x41, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{sp_el2|SP_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{SPSel|SPSEL}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x42, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{SPSel|SPSEL}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{nzcv|NZCV}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x42, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{nzcv|NZCV}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{daif|DAIF}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x42, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{daif|DAIF}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{CurrentEL|CURRENTEL}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x42, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{CurrentEL|CURRENTEL}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{SPSR_irq|SPSR_IRQ}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x43, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{SPSR_irq|SPSR_IRQ}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{SPSR_abt|SPSR_ABT}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x43, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{SPSR_abt|SPSR_ABT}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{SPSR_und|SPSR_UND}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x43, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{SPSR_und|SPSR_UND}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{SPSR_fiq|SPSR_FIQ}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0x43, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{SPSR_fiq|SPSR_FIQ}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{fpcr|FPCR}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x44, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{fpcr|FPCR}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{fpsr|FPSR}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x44, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{fpsr|FPSR}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{dspsr_el0|DSPSR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x45, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{dspsr_el0|DSPSR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{dlr_el0|DLR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x45, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{dlr_el0|DLR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{ifsr32_el2|IFSR32_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x50, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{ifsr32_el2|IFSR32_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{afsr0_el1|AFSR0_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x51, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{afsr0_el1|AFSR0_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{afsr0_el2|AFSR0_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x51, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{afsr0_el2|AFSR0_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{afsr0_el3|AFSR0_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x51, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{afsr0_el3|AFSR0_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{afsr1_el1|AFSR1_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x51, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{afsr1_el1|AFSR1_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{afsr1_el2|AFSR1_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x51, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{afsr1_el2|AFSR1_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{afsr1_el3|AFSR1_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x51, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{afsr1_el3|AFSR1_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{esr_el1|ESR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x52, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{esr_el1|ESR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{esr_el2|ESR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x52, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{esr_el2|ESR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{esr_el3|ESR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x52, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{esr_el3|ESR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{fpexc32_el2|FPEXC32_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x53, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{fpexc32_el2|FPEXC32_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{far_el1|FAR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x60, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{far_el1|FAR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{far_el2|FAR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x60, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{far_el2|FAR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{far_el3|FAR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x60, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{far_el3|FAR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{hpfar_el2|HPFAR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0x60, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{hpfar_el2|HPFAR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{par_el1|PAR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x74, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{par_el1|PAR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmcr_el0|PMCR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x9c, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmcr_el0|PMCR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmcntenset_el0|PMCNTENSET_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x9c, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmcntenset_el0|PMCNTENSET_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmcntenclr_el0|PMCNTENCLR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x9c, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmcntenclr_el0|PMCNTENCLR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmovsclr_el0|PMOVSCLR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0x9c, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmovsclr_el0|PMOVSCLR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmselr_el0|PMSELR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0x9c, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmselr_el0|PMSELR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmceid0_el0|PMCEID0_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0x9c, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmceid0_el0|PMCEID0_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmceid1_el0|PMCEID1_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0x9c, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmceid1_el0|PMCEID1_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmccntr_el0|PMCCNTR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x9d, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmccntr_el0|PMCCNTR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmxevtyper_el0|PMXEVTYPER_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x9d, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmxevtyper_el0|PMXEVTYPER_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmxevcntr_el0|PMXEVCNTR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x9d, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmxevcntr_el0|PMXEVCNTR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmuserenr_el0|PMUSERENR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0x9e, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmuserenr_el0|PMUSERENR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmintenset_el1|PMINTENSET_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0x9e, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmintenset_el1|PMINTENSET_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmintenclr_el1|PMINTENCLR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0x9e, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmintenclr_el1|PMINTENCLR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmovsset_el0|PMOVSSET_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0x9e, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmovsset_el0|PMOVSSET_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mair_el1|MAIR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xa2, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mair_el1|MAIR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mair_el2|MAIR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xa2, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mair_el2|MAIR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{mair_el3|MAIR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xa2, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{mair_el3|MAIR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{amair_el1|AMAIR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xa3, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{amair_el1|AMAIR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{amair_el2|AMAIR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xa3, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{amair_el2|AMAIR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{amair_el3|AMAIR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xa3, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{amair_el3|AMAIR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{vbar_el1|VBAR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xc0, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{vbar_el1|VBAR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{vbar_el2|VBAR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xc0, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{vbar_el2|VBAR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{vbar_el3|VBAR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xc0, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{vbar_el3|VBAR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{rvbar_el1|RVBAR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xc0, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{rvbar_el1|RVBAR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{rvbar_el2|RVBAR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xc0, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{rvbar_el2|RVBAR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{rvbar_el3|RVBAR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xc0, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{rvbar_el3|RVBAR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{rmr_el1|RMR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xc0, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{rmr_el1|RMR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{rmr_el2|RMR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xc0, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{rmr_el2|RMR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{rmr_el3|RMR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xc0, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{rmr_el3|RMR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{isr_el1|ISR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xc1, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{isr_el1|ISR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{contextidr_el1|CONTEXTIDR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xd0, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{contextidr_el1|CONTEXTIDR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{tpidr_el0|TPIDR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xd0, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{tpidr_el0|TPIDR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{tpidr_el2|TPIDR_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xd0, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{tpidr_el2|TPIDR_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{tpidr_el3|TPIDR_EL3}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xd0, 0x3e, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{tpidr_el3|TPIDR_EL3}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{tpidrro_el0|TPIDRRO_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xd0, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{tpidrro_el0|TPIDRRO_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{tpidr_el1|TPIDR_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xd0, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{tpidr_el1|TPIDR_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntfrq_el0|CNTFRQ_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe0, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntfrq_el0|CNTFRQ_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntpct_el0|CNTPCT_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xe0, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntpct_el0|CNTPCT_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntvct_el0|CNTVCT_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xe0, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntvct_el0|CNTVCT_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntvoff_el2|CNTVOFF_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xe0, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntvoff_el2|CNTVOFF_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntkctl_el1|CNTKCTL_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe1, 0x38, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntkctl_el1|CNTKCTL_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cnthctl_el2|CNTHCTL_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe1, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cnthctl_el2|CNTHCTL_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntp_tval_el0|CNTP_TVAL_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe2, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntp_tval_el0|CNTP_TVAL_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cnthp_tval_el2|CNTHP_TVAL_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe2, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cnthp_tval_el2|CNTHP_TVAL_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntps_tval_el1|CNTPS_TVAL_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe2, 0x3f, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntps_tval_el1|CNTPS_TVAL_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntp_ctl_el0|CNTP_CTL_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xe2, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntp_ctl_el0|CNTP_CTL_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cnthp_ctl_el2|CNTHP_CTL_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xe2, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cnthp_ctl_el2|CNTHP_CTL_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntps_ctl_el1|CNTPS_CTL_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xe2, 0x3f, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntps_ctl_el1|CNTPS_CTL_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntp_cval_el0|CNTP_CVAL_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xe2, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntp_cval_el0|CNTP_CVAL_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cnthp_cval_el2|CNTHP_CVAL_EL2}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xe2, 0x3c, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cnthp_cval_el2|CNTHP_CVAL_EL2}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntps_cval_el1|CNTPS_CVAL_EL1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xe2, 0x3f, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntps_cval_el1|CNTPS_CVAL_EL1}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntv_tval_el0|CNTV_TVAL_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe3, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntv_tval_el0|CNTV_TVAL_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntv_ctl_el0|CNTV_CTL_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xe3, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntv_ctl_el0|CNTV_CTL_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{cntv_cval_el0|CNTV_CVAL_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xe3, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{cntv_cval_el0|CNTV_CVAL_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr0_el0|PMEVCNTR0_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe8, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr0_el0|PMEVCNTR0_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr1_el0|PMEVCNTR1_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xe8, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr1_el0|PMEVCNTR1_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr2_el0|PMEVCNTR2_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xe8, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr2_el0|PMEVCNTR2_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr3_el0|PMEVCNTR3_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xe8, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr3_el0|PMEVCNTR3_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr4_el0|PMEVCNTR4_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xe8, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr4_el0|PMEVCNTR4_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr5_el0|PMEVCNTR5_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0xe8, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr5_el0|PMEVCNTR5_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr6_el0|PMEVCNTR6_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0xe8, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr6_el0|PMEVCNTR6_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr7_el0|PMEVCNTR7_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0xe8, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr7_el0|PMEVCNTR7_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr8_el0|PMEVCNTR8_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xe9, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr8_el0|PMEVCNTR8_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr9_el0|PMEVCNTR9_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xe9, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr9_el0|PMEVCNTR9_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr10_el0|PMEVCNTR10_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xe9, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr10_el0|PMEVCNTR10_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr11_el0|PMEVCNTR11_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xe9, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr11_el0|PMEVCNTR11_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr12_el0|PMEVCNTR12_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xe9, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr12_el0|PMEVCNTR12_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr13_el0|PMEVCNTR13_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0xe9, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr13_el0|PMEVCNTR13_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr14_el0|PMEVCNTR14_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0xe9, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr14_el0|PMEVCNTR14_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr15_el0|PMEVCNTR15_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0xe9, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr15_el0|PMEVCNTR15_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr16_el0|PMEVCNTR16_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xea, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr16_el0|PMEVCNTR16_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr17_el0|PMEVCNTR17_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xea, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr17_el0|PMEVCNTR17_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr18_el0|PMEVCNTR18_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xea, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr18_el0|PMEVCNTR18_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr19_el0|PMEVCNTR19_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xea, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr19_el0|PMEVCNTR19_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr20_el0|PMEVCNTR20_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xea, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr20_el0|PMEVCNTR20_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr21_el0|PMEVCNTR21_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0xea, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr21_el0|PMEVCNTR21_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr22_el0|PMEVCNTR22_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0xea, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr22_el0|PMEVCNTR22_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr23_el0|PMEVCNTR23_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0xea, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr23_el0|PMEVCNTR23_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr24_el0|PMEVCNTR24_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xeb, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr24_el0|PMEVCNTR24_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr25_el0|PMEVCNTR25_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xeb, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr25_el0|PMEVCNTR25_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr26_el0|PMEVCNTR26_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xeb, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr26_el0|PMEVCNTR26_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr27_el0|PMEVCNTR27_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xeb, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr27_el0|PMEVCNTR27_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr28_el0|PMEVCNTR28_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xeb, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr28_el0|PMEVCNTR28_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr29_el0|PMEVCNTR29_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0xeb, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr29_el0|PMEVCNTR29_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevcntr30_el0|PMEVCNTR30_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0xeb, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevcntr30_el0|PMEVCNTR30_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmccfiltr_el0|PMCCFILTR_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0xef, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmccfiltr_el0|PMCCFILTR_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper0_el0|PMEVTYPER0_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xec, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper0_el0|PMEVTYPER0_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper1_el0|PMEVTYPER1_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xec, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper1_el0|PMEVTYPER1_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper2_el0|PMEVTYPER2_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xec, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper2_el0|PMEVTYPER2_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper3_el0|PMEVTYPER3_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xec, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper3_el0|PMEVTYPER3_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper4_el0|PMEVTYPER4_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xec, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper4_el0|PMEVTYPER4_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper5_el0|PMEVTYPER5_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0xec, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper5_el0|PMEVTYPER5_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper6_el0|PMEVTYPER6_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0xec, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper6_el0|PMEVTYPER6_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper7_el0|PMEVTYPER7_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0xec, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper7_el0|PMEVTYPER7_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper8_el0|PMEVTYPER8_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xed, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper8_el0|PMEVTYPER8_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper9_el0|PMEVTYPER9_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xed, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper9_el0|PMEVTYPER9_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper10_el0|PMEVTYPER10_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xed, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper10_el0|PMEVTYPER10_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper11_el0|PMEVTYPER11_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xed, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper11_el0|PMEVTYPER11_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper12_el0|PMEVTYPER12_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xed, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper12_el0|PMEVTYPER12_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper13_el0|PMEVTYPER13_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0xed, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper13_el0|PMEVTYPER13_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper14_el0|PMEVTYPER14_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0xed, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper14_el0|PMEVTYPER14_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper15_el0|PMEVTYPER15_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0xed, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper15_el0|PMEVTYPER15_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper16_el0|PMEVTYPER16_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xee, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper16_el0|PMEVTYPER16_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper17_el0|PMEVTYPER17_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xee, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper17_el0|PMEVTYPER17_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper18_el0|PMEVTYPER18_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xee, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper18_el0|PMEVTYPER18_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper19_el0|PMEVTYPER19_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xee, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper19_el0|PMEVTYPER19_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper20_el0|PMEVTYPER20_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xee, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper20_el0|PMEVTYPER20_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper21_el0|PMEVTYPER21_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0xee, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper21_el0|PMEVTYPER21_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper22_el0|PMEVTYPER22_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0xee, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper22_el0|PMEVTYPER22_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper23_el0|PMEVTYPER23_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe9, 0xee, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper23_el0|PMEVTYPER23_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper24_el0|PMEVTYPER24_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x09, 0xef, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper24_el0|PMEVTYPER24_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper25_el0|PMEVTYPER25_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x29, 0xef, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper25_el0|PMEVTYPER25_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper26_el0|PMEVTYPER26_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x49, 0xef, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper26_el0|PMEVTYPER26_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper27_el0|PMEVTYPER27_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x69, 0xef, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper27_el0|PMEVTYPER27_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper28_el0|PMEVTYPER28_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x89, 0xef, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper28_el0|PMEVTYPER28_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper29_el0|PMEVTYPER29_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xa9, 0xef, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper29_el0|PMEVTYPER29_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x9, {{pmevtyper30_el0|PMEVTYPER30_EL0}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xc9, 0xef, 0x3b, 0xd5}),
				address:          0,
			},
			want: "mrs	x9, {{pmevtyper30_el0|PMEVTYPER30_EL0}}",
			wantErr: false,
		},
		{
			name: "mrs	x12, {{s3_7_c15_c1_5|S3_7_C15_C1_5}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xac, 0xf1, 0x3f, 0xd5}),
				address:          0,
			},
			want: "mrs	x12, {{s3_7_c15_c1_5|S3_7_C15_C1_5}}",
			wantErr: false,
		},
		{
			name: "mrs	x13, {{s3_2_c11_c15_7|S3_2_C11_C15_7}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xed, 0xbf, 0x3a, 0xd5}),
				address:          0,
			},
			want: "mrs	x13, {{s3_2_c11_c15_7|S3_2_C11_C15_7}}",
			wantErr: false,
		},
		{
			name: "mrs	x14, {{s1_3_c9_c2_1|S1_3_C9_C2_1}}",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x2e, 0x92, 0x2b, 0xd5}),
				address:          0,
			},
			want: "mrs	x14, {{s1_3_c9_c2_1|S1_3_C9_C2_1}}",
			wantErr: false,
		},
		{
			name: "msr	{{s3_0_c15_c0_0|S3_0_C15_C0_0}}, x12",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x0c, 0xf0, 0x18, 0xd5}),
				address:          0,
			},
			want: "msr	{{s3_0_c15_c0_0|S3_0_C15_C0_0}}, x12",
			wantErr: false,
		},
		{
			name: "msr	{{s3_7_c11_c13_7|S3_7_C11_C13_7}}, x5",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0xe5, 0xbd, 0x1f, 0xd5}),
				address:          0,
			},
			want: "msr	{{s3_7_c11_c13_7|S3_7_C11_C13_7}}, x5",
			wantErr: false,
		},
		{
			name: "msr	{{s1_3_c9_c2_1|S1_3_C9_C2_1}}, x4",
			args: args{
				instructionValue: binary.LittleEndian.Uint32([]byte{0x24, 0x92, 0x0b, 0xd5}),
				address:          0,
			},
			want: "msr	{{s1_3_c9_c2_1|S1_3_C9_C2_1}}, x4",
			wantErr: false,
		},
		{
			name: "b	#4",
